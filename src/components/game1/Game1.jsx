...
// Count Chinese characters in a word
function charCount(word) {
  return word.replace(/\s/g, '').length;
}

// Count pinyin syllables (each Chinese char = 1 syllable)
function syllableCount(word) {
  return charCount(word);
}

// Generate distractors matching the same character count
function getDistractors(correctWord, allWords) {
  const targetChars = charCount(correctWord.char);
  const pool = allWords.filter(
    (w) => w.id !== correctWord.id && charCount(w.char) === targetChars
  );
  ...
  return { options, correctIndex: options.findIndex((o) => o.id === correctWord.id) };
}

function buildQuestions(allWords) {
  const shuffled = [...allWords].sort(() => Math.random() - 0.5);
  return shuffled.slice(0, 30).map((word) => {
    const { options, correctIndex } = getDistractors(word, allWords);
...
export default function Game1({ onHome, hskLevel = '3' }) {
  const vocabulary = hskLevel === '4' ? hsk4Vocabulary : hsk3Vocabulary;
  const recordGame1 = useGameStore((s) => s.recordGame1);
  ...
  const handleSelect = useCallback(
    (idx) => {
      ...
      setAnswers((prev) => [...prev, { correct: q.correctIndex, selected: idx, word: q.word }]);
        ...
        setTimeout(() => {
          setCurrent((prev) => {
            if (prev + 1 >= QUESTIONS_PER_ROUND) {
              recordGame1(score + 1, QUESTIONS_PER_ROUND);
              setShowResult(true);
              return prev;
            }
            return prev + 1;
  ...
  const handleNext = useCallback(() => {
    ...
    if (current + 1 >= QUESTIONS_PER_ROUND) {
      recordGame1(finalScore, QUESTIONS_PER_ROUND);
      setShowResult(true);
    } else {
      setCurrent((c) => c + 1);
      setSelected(null);
    }
  }, [current, score, selected, q, recordGame1]);
  ...
  return (
    <div className="flex flex-col gap-6 max-w-md mx-auto py-8 px-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-ink-800">🔤 Character → Pinyin</h2>
          <p className="text-sm text-ink-400">Match the correct pinyin</p>
        </div>
      ...
      <div className="card p-10 flex flex-col items-center gap-3 bg-gradient-to-br from-white to-ink-50">
        <div className="text-sm text-ink-400">What is the pinyin?</div>
        <div className="chinese-char font-chinese text-6xl">{q.word.char}</div>
        <div className="text-xs text-ink-300 mt-2">HSK 3 • #{q.word.id}</div>
      </div>
        ...
        {q.options.map((opt, idx) => {
          ...
          return (
            <button key={idx} className={cls} onClick={() => handleSelect(idx)}>
              ...
              <span className="font-chinese text-lg">{opt.pinyin}</span>
            </button>
          );
        })}
      </div>
      ...
      {/* Feedback */}
      {answered && (
        <div className={`card p-4 border-2 ${isCorrect ? 'border-emerald-300 bg-emerald-50' : 'border-red-300 bg-red-50'}`}>
            ...
            <div>
              ...
              {!isCorrect && (
                <div className="text-sm text-ink-600 mt-1">
                  ...
                  <span className="font-semibold text-emerald-700">{q.options[q.correctIndex].pinyin}</span>{' '}
                  <span className="text-ink-400">— {q.word.en}</span>
                </div>
              )}
            </div>
          </div>
